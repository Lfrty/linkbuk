...
export class ListaService {
  ...
  getListas() {
    this.http.get<{ data: Lista[] }>(API.listas.listar).subscribe({
      next: (res) => {
  ...
  crearLista(nombre: string) {
    return this.http.post(API.listas.crear, { nombre }).pipe(
      tap(() => this.getListas())
    );
  }

  updateLista(id: any, nombre: string) {
    return this.http.post(API.listas.actualizar(id), { nombre }).pipe(
      tap(() => this.getListas())
    );
  }
  ...
  addLibro(listaId: number, libroId: number) {
    return this.http.post(API.listas.addLibro(listaId), { libro_id: libroId }).pipe(
      tap(() => this.getListas())
    );
  }

  quitarLibro(listaId: number, libroId: number) {
    return this.http.delete(`${API.listas.eliminarLibro(listaId, libroId)}`).pipe(
      tap(() => this.getListas())
    );
  }
